...
** The left hand should be sufficient when the other hand is on the mouse.
Meaning that:
+ All modifiers should be accessible :: control-click, shift-click, C-w (close tab) C-t (new tab) C-tab (tab next), etc.
      - This imply a way to mirror the right hand on the left to do things like C-y (in Emacs)
+ backspace should be accessible :: Select some text and delete it.

This imply that I should dedicate the inner left thumb key to the layer with one shot homerow mods.
Could backspace be on the outer left thunb when on that homerow mod layer ?
...
** TODO Find a spot for ^
** TODO Put Tab where QK_BOOT is and find a new spot for QK_BOOT
** TODO In Emacs, set C-t as tab-new
** TODO Write a udev rule for french layout when keyboard is plugged in. us-intl when unplugged
** TODO Closing parenthesis followed by space is uncomfortable.
* Navigation layer
The navigation layer serves two purposes:
+ When holding the modifier, the right hand keys allow to navigate a document.
+ Using the trackpad activates this layer and allows the left hand to use functionalities like copy, cut, paste, etc.

* French accents
I like to use combos because it doesn't require an additional keypress. The trick is to find comfortable combos.
https://bepo.fr/wiki/Fr%C3%A9quence_des_digrammes

** Accent aigu
We give the ~é~ its own key on the base layer, at the same location than the french canadian keyboard layout.

** Accent grave
+ À :: Mostly preceded by a consonant and followed by a punctuation sign.
+ È :: Mostly followed and preceded by consonants, though ~iè~ (/lièvre/), ~oè~ (/poème/) happens.
+ Conclusion :: ~<letter>+n~ might be a good idea. *Important*: auto shift must be enabled for this to work, otherwise typing À sucks.

** Accent circonflexe
Let's keep ~<letter>+x~ as I'm already used to it but because ~a+x~ combo sucks, we'll mirror it for ~a+.~. Hopefully this won't be a problem when typing fast and ending a sentence with ~a.~.

** Tréma
This one is very rare and we are lucky because the three vowels that can have it are on the same half of the keyboard. So we'll stick for ~<letter>+d~ for this one.

** Cédille
Because ~ç~ is always followed by another vowel, ~c+d~ works very well.
...
Some other symbols come in pairs, such as «», (), {}, []. Macros might be better suited to deal with these.
...
** CANCELLED As keys on another layer
+ Two keypresses
